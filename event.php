...
<div class="reg" align="center"><br>
        <h3 class="form-head">Select Events</h3><br>
    <form method="post" autocomplete="off">
        <div class="form-block">
        <table>
        <tr>
        <td><label for="event" class="control-label">Select Department</label></td>
        <td>
        <select name="event" id="event" class="form-control" required>
            <option value=''>Select</option>
            <?php
            include('db.php');
            $sql = "SELECT event FROM admin_exdetails GROUP BY event ORDER BY event ASC";
...
{
    $event= $_POST["event"];
    $sql = "SELECT * FROM admin_exdetails WHERE event='$event' ORDER BY event";
    $result = mysqli_query($con, $sql);
    if($result)
    {
